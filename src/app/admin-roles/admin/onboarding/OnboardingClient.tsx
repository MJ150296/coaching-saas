...
export default function OnboardingFlowPage() {
  ...
  const [academicYearSearch] = useState('');
  const [classMasterId, setClassMasterId] = useState('');
  const [classMasterSearch] = useState('');
  const [sectionId, setSectionId] = useState('');
  const [studentSearch] = useState('');
  const [feeTypeSearch] = useState('');
  const [feePlanSearch] = useState('');
  ...
  const [sectionForm, setSectionForm] = useState({ name: '', capacity: '', roomNumber: '', shift: '' });
  const [subjectForm, setSubjectForm] = useState({ subjectName: '', weeklyPeriods: '' });
  ...
  async function fetchClassMasters(orgId?: string, schId?: string) {
    try {
      ...
      return data
        ...
        .map((row) => ({
          ...
          level: typeof row.classLevel === 'string' ? row.classLevel : undefined,
  ...
  }, [statusMap, organizationId, coachingCenterId, academicYearId, teacherId, feePlanId, studentId, skipAssignFeePlan]);
  ...
  const latestAcademicYearId = useMemo(() => {
    const scoped = academicYears.filter(
      (item) => item.organizationId === organizationId && item.coachingCenterId === coachingCenterId
    );
    ...
    return [...scoped]
      .sort((a, b) => {
        const byStartDate = getDateRank(b.startDate) - getDateRank(a.startDate);
        if (byStartDate !== 0) return byStartDate;

        const byEndDate = getDateRank(b.endDate) - getDateRank(a.endDate);
        if (byEndDate !== 0) return byEndDate;

        return b.name.localeCompare(a.name);
      })[0]?.id ?? '';
  }, [academicYears, organizationId, coachingCenterId]);
  ...
  const loadClassMasters = useCallback(async (orgId?: string, schId?: string) => {
  ...
  async function loadTeachers(orgId?: string, schId?: string) {
    ...
    try {
      await fetchTeachers(orgId, schId);
    } finally {
  ...
  }, [organizationId, coachingCenterId, loadClassMasters]);

  useEffect(() => {
    if (!organizationId || !coachingCenterId) return;
    if (!latestAcademicYearId) return;

    const currentSelectionIsValid = academicYears.some(
      (item) =>
        item.id === academicYearId &&
        item.organizationId === organizationId &&
        item.coachingCenterId === coachingCenterId
    );

    if (!academicYearId || !currentSelectionIsValid) {
      setAcademicYearId((prev) => (prev === latestAcademicYearId ? prev : latestAcademicYearId));
    }
  }, [academicYearId, academicYears, coachingCenterId, latestAcademicYearId, organizationId]);
      ...
      case 5:
        return (
          <TeacherClassTeacherStep
            ...
            programId={classMasterId}
            setProgramId={setClassMasterId}
            tenantProgramOptions={tenantClassMasterOptions}
            onRefreshPrograms={() => loadClassMasters(organizationId, coachingCenterId)}
            ...
            onCreate={() => {
              ...
              postStep(
                ...
                '/api/admin/users',
                {
                  ...
                  schoolGrade: studentForm.schoolGrade || undefined,
                  ...
                  parent: {
                    ...
                    password: studentForm.parentPassword,
