...
type CoachingAdminDashboardClientProps = {
...
export default function CoachingAdminDashboardClient({
  initialStats,
  initialRecentUsers,
  organizationId,
  coachingCenterId,
  coachingCenterName,
}: CoachingAdminDashboardClientProps) {
  ...
  const [centerName, setCenterName] = useState(coachingCenterName ?? coachingCenterId);

  const loadDashboardData = useCallback(async () => {
    setLoading(true);
    try {
      const [overviewResponse, coachingCenters] = await Promise.all([
        fetch('/api/admin/dashboard/overview'),
        getAdminCoachingCenters(organizationId, { force: true }),
      ]);
      ...
      const currentCoachingCenter = coachingCenters.find((item) => item.id === coachingCenterId);
      ...
      setCenterName(currentCoachingCenter?.name ?? coachingCenterId);
    } catch (error) {
      toastMessage(`Error: ${String(error)}`);
    } finally {
      setLoading(false);
    }
  }, [organizationId, coachingCenterId, toastMessage]);
  ...
  return (
    <div className="bg-linear-to-br from-slate-50 via-emerald-50/25 to-lime-50/45 py-8">
        ...
        <section className="rounded-2xl border border-emerald-100 bg-linear-to-r from-emerald-600 via-teal-600 to-lime-600 p-6 shadow-lg shadow-emerald-200/70">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant="blue">Org: {organizationId}</Badge>
              <Badge variant="green">Coaching Center: {centerName}</Badge>
            </div>
          </div>
        </section>
