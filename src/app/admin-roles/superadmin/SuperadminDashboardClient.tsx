...
import type { OrganizationItem, CoachingCenterItem } from '@/shared/lib/superadmin-dashboard.server';
import type { AdminDashboardOverviewPayload } from '@/shared/lib/admin-dashboard.server';

type SuperadminDashboardClientProps = {
  initialOverview: AdminDashboardOverviewPayload;
  initialOrganizations: OrganizationItem[];
  initialCoachingCenters: CoachingCenterItem[];
...
export default function SuperadminDashboardClient({
  ...
  const [coachingCenters, setCoachingCenters] = useState<CoachingCenterItem[]>(initialCoachingCenters);
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      setOverview({
        ...
        scope: overviewBody?.scope ?? initialOverview.scope,
      });
  ...
  }, [toastMessage, initialOverview.scope]);
  ...
  return (
    <div className="bg-linear-to-br from-slate-50 via-violet-50/30 to-indigo-50/40 py-8">
